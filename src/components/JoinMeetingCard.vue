<template>
	<div class="join-meeting">
		<div class="bg-white rounded-px5 p-px25 shadow-custom xl:p-10">
			<h2 class="font-bold text-grey-dark text-2xl xl:text-3xl text-center mb-5 xl:mb-px30">Join Meeting</h2>
			<form @submit.prevent="joinMeeting" class="flex flex-col gap-2.5 xl:gap-5 px-2.5 text-xs xl:text-sm">
				<div class="flex flex-col gap">
                    ...
                    <span v-if="v$.joinMeetingData.meetId.$error" class="text-red text-px8 xl:text-px10 mt-px5">{{ v$.joinMeetingData.meetId.$errors[0].$message }}</span>
				</div>
				<div v-if="isAuth === false" class="flex flex-col gap">
                    ...
                    <span v-if="v$.joinMeetingData.name.$error" class="text-red text-px8 xl:text-px10 mt-px5">{{ v$.joinMeetingData.name.$errors[0].$message }}</span>
				</div>
				...
				<div class="flex flex-col gap">
                    ...
                    <span v-if="v$.joinMeetingData.meetPassword.$error" class="text-red text-px8 xl:text-px10 mt-px5">{{ v$.joinMeetingData.meetPassword.$errors[0].$message }}</span>
				</div>
			...
			<p v-if="isAuth === false" class="text-xs xl:text-sm text-grey-dark my-2.5 ml-2.5">Are you a host? 
