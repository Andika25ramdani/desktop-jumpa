<template>
	<div id="createNewtPassword">
		<div class="flex flex-col items-center bg-white rounded-px5 p-px30 shadow-custom xl:p-10">
			<h2 class="font-bold text-grey-dark text-2xl xl:text-3xl text-center mb-2.5">Create New Password</h2>
            <img src="img/key-password.png" alt="" class="mx-auto py-px30">
            <p class="text-xs xl:text-sm text-grey-dark text-center mb-px30">Set the new password for your account</p>
            <form @submit.prevent="createNewPassword" class="flex flex-col gap-2.5 xl:gap-5 px-2.5 text-xs xl:text-sm">
                ...
                <div class="flex flex-col gap">
                    ...
                    <span v-if="v$.newPasswordData.password.$error" class="text-red text-px8 xl:text-px10 mt-px5">{{ v$.newPasswordData.password.$errors[0].$message }}</span>
                    <span v-else class="text-px8 xl:text-px10 text-grey-sb mt-px5">8-16 characters, contain two types of digits, letters or symbols</span>
				</div>
				<div class="flex flex-col gap">
					<div class="flex gap-px5 border border-grey-ce rounded-px5 red-input">
                    ...
                    <span v-if="v$.newPasswordData.confirmPassword.$error" class="text-red text-px8 xl:text-px10 mt-px5">{{ v$.newPasswordData.confirmPassword.$errors[0].$message }}</span>
				</div>
