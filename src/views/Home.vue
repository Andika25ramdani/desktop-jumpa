  ...
  <div class="fixed flex flex-col sm:flex-row justify-between w-full">
    <div class="home-ls">
		<transition-group name="slide-left" appear>
			<h3 key="0" class="kanit font-bold text-xl lg:text-3xl text-grey-black mt-4 mb-px5 xl:mb-px15 px-4 sm:px-16">Hi, {{ userInfo.displayName }}</h3>
			<p key="1" class="text-px10 xl:text-base text-grey-gray mb-5 xl:mb-px30 px-4 sm:px-16">{{ userInfo.bio }}</p>
			<div key="2" class="mx-auto bg-white w-max py-px15 px-5 xl:p-px25 border-grey-disable border rounded-px10 flex items-center justify-evenly gap-5 xl:gap-6">
				<div @click="newMeeting" class="flex flex-col items-center gap-2.5 w-max cursor-pointer">
					<div class="bg-maroon rounded-px10 w-16 xl:w-px100 h-16 xl:h-px100 p-px13 xl:p-7 flex items-center">
			...
			<div v-if="currentTab === 'upcomingMeeting'" class="h-full pt-2.5 xl:pt-px30 px-2.5 md:px-px25 xl:px-px45">
				<div id="upcomingList" class="h-full pb-32 grid grid-cols-1 gap-2.5 xl:gap-5 sticky flex-col flex-wrap overflow-hidden overflow-y-auto hasInactive">
					<transition-group name="slide-right" v-if="upcomingMeetings.length > 1" appear>
						<div v-for="upcoming in upcomingMeetings" :key=upcoming.meetingNum class="sch-upc-meeting rounded-px10 bg-maroon text-white p-px15 xl:p-5 relative">
							<div class="flex gap-2.5 xl:gap-px15 items-center text-px8 xl:text-xs">
								<p class="text-white">Meeting ID: {{ upcoming.meetingNum }}</p>
								<p v-if="upcoming.state === 2" class="bg-lightgreen rounded-full py-0.5 xl:py-1 px-4">In Process</p>
