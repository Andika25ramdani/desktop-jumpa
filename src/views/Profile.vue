            ...
            <div class="flex flex-nowrap gap-px35 xl:gap-15">
                <div class="h-full relative flex flex-col items-center gap-5 xl:gap-10">
                    <figure class="relative">
                        <img :src="userInfo.image" class="rounded-full w-36 xl:w-48 h-36 xl:h-48 object-contain bg-grey-dark">
                    </figure>
                    <div class="relative">
                        <button @click="editPhotoMenu = !editPhotoMenu" class="bg-grey-background border border-grey-lighter text-px10 xl:text-sm text-grey-dark rounded-px5 px-4 py-px5">
                        ...
                        <transition name="slide-left" appear>
                            <div v-if="editPhotoMenu == true" class="flex flex-col rounded-px5 shadow-sm bg-white py-2.5 px-px15 absolute -top-4 left-1/3 gap-px5 lg:gap-2 inactive">
                                <p class="text-px10 xl:text-sm text-grey-sb cursor-pointer hover:text-grey-ed hover:font-bold w-max">Take photo</p>
                                <form @submit.prevent="submitData" name="formGambar" class="flex gap-0">
                                    ...
                                    <label for="inputImage" class="text-px10 xl:text-sm text-grey-sb cursor-pointer hover:text-grey-ed hover:font-bold w-max">Upload photo</label>
                                    <!-- <input type="submit" value="Upload Photo"> -->
                                </form>
                                <p class="text-px10 xl:text-sm text-grey-sb cursor-pointer hover:text-grey-ed hover:font-bold w-max">Remove photo</p>
                            </div>
                        </transition>
                    </div>
                </div>
